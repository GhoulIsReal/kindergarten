...
const Form = () => {
    ...
    const handleFormSubmitButton = () => {
        ...
        window.fetch("http://localhost:3001/request", {
            method: 'POST',
            headers: {
    ...
    const formValidate = () => {
        if(inputNumber.current.value.length > 10){
            ...
            inputNumber.current.placeholder = 'Заполните данное поле(мин. 11 символов)';
        }
    }

    return(
        <styled.MainScreenFormHolder>
            <styled.MainScreenInput ref={inputName} type="text" placeholder="Имя и Фамилия" id="NameAndSurname"></styled.MainScreenInput>
            <styled.MainScreenInput ref={inputNumber} type="tel" placeholder="Телефон(+7 (777) 792-848-24)" id="PhoneNum" required></styled.MainScreenInput>
            <styled.MainScreenTextArea ref={textArea} placeholder="Можете оставить комментарий...(макс 150 символов)" maxLength="150" id="Comments"></styled.MainScreenTextArea>
            <styled.MainScreenAlertAndSendButtonHolder>
                <styled.MainScreenFormTextAlert>
                    <styled.FormAlertTextHolder ref={alert}><FormattedMessage id="mainScreen.formAlert"/></styled.FormAlertTextHolder>
                    <styled.EmptyDiv ref={statusDiv} className="loaderDiv">{response}</styled.EmptyDiv>
                ...
                <styled.MainScreenAbbreviation title="Нажимая кнопку 'Отправить' Вы даете согласие на обработку своих персональных данных">
                    <styled.MainScreenSendButton onClick={formValidate} primary type="submit" value="Отправить"></styled.MainScreenSendButton>
